...
import { STUBBED_GOOGLE_VERIFYID, TOKEN_RESPONSE_STRUCTURE, STUBBED_FACEBOOK_VERIFYID } from './constants';
...
describe('Google Auth', () => {
  ...
  it('should return user already registered [register]', (done) => {
    const stub = sinon.stub(FacebookService.prototype, 'verifyToken').callsFake((token) => {
      return STUBBED_FACEBOOK_VERIFYID;
    });
  ...
  it('should return 200 user information [register]', (done) => {
    FacebookService.prototype.verifyToken.restore();
    const stub = sinon.stub(FacebookService.prototype, 'verifyToken').callsFake((token) => {
      return {
