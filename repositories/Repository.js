...
export default class Repository {
  /**
 * Base repository class providing common used operations
 * accessing the data layer.n
 * @param {Model} model
 */
  constructor(model) {
    this.model = model;
  ...
  async paginate(req, page = 1, limit, orders, attributes) {
    ...
    const order = typeof orders !== 'undefined' ? Repository.parseOrder(orders) : ['id', 'ASC'];
    ...
    try {
      ...
      try {
        ...
        const links = await Repository.generateLinks(page, lastPage, req.baseUrl);
        if (response) {
          return {
            data: response.rows,
            links,
