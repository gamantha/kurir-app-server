...
describe('Google Auth', function () {
  ...
  it('should return user already registered [register]', function (done) {
    var stub = _sinon2.default.stub(_index.FacebookService.prototype, 'verifyToken').callsFake(function (token) {
      return _constants.STUBBED_FACEBOOK_VERIFYID;
    });

    _chai2.default.request(_app2.default).post('/api/facebook/register').send({
      tokenId: 'randomtoken'
  ...
  it('should return logged in user token [login]', function (done) {
    _chai2.default.request(_app2.default).post('/api/facebook/login').send({
      tokenId: 'randomtoken'
    }).end(function (err, res) {
      res.should.have.status(200);
      res.body.should.include.keys(baseResponseStructure);
  ...
  it('should return 200 user information [register]', function (done) {
    _index.FacebookService.prototype.verifyToken.restore();
    var stub = _sinon2.default.stub(_index.FacebookService.prototype, 'verifyToken').callsFake(function (token) {
      return {
